...
function checkCacheVersion() {
  ...
  if (storedVersion !== CACHE_VERSION) {
    console.log(`缓存版本更新: ${storedVersion} -> ${CACHE_VERSION}, 清理旧缓存`)
    dictCache.clear()
...
export async function getDictItems(dictCode, useCache = true, options = {}) {
  ...
  if (cacheEntry && isCacheValid(cacheEntry)) {
    console.log(`使用字典缓存: ${dictCode}`)
    return cacheEntry.data
  }
  
  // 缓存未命中或已过期，重新获取
  try {
    console.log(`字典缓存未命中，重新获取: ${dictCode}`)
    const response = await getDictItemsByDictCode(dictCode, 1)
    const items = response.success ? response.data : []
...
export function clearDictCache(dictCode, options = {}) {
  ...
  if (dictCode) {
    ...
    if (deleted) {
      console.log(`清除字典缓存: ${dictCode}`)
    }
  } else {
    // 清除所有缓存
    const size = dictCache.size
    dictCache.clear()
    
    if (force) {
      // 强制清理时，重置版本标识
      localStorage.removeItem(CACHE_VERSION_KEY)
      console.log(`强制清除所有字典缓存 (${size}条) 并重置版本`)
    } else {
      console.log(`清除所有字典缓存 (${size}条)`)
    }
  }
}
