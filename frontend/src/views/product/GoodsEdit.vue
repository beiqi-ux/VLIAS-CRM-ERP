      ...
      >
        ...
        <el-card class="form-section" shadow="never">
          ...
          <el-row>
            <el-col :span="24">
              ...
              <!-- 空状态 -->
              <div v-if="specificationGroups.length === 0" class="spec-empty-state">
                <div class="spec-empty-content">
                  <div class="spec-empty-icon">
                    <el-icon size="40" color="#c8c9cc"><Setting /></el-icon>
                  </div>
                  <div class="spec-empty-text">
                    <div class="spec-empty-title">暂未设置商品规格</div>
                    <div class="spec-empty-desc">为商品添加规格属性，如颜色、尺寸、材质等，支持多种规格组合</div>
                  </div>
                </div>
                <el-button 
                  type="primary" 
              ...
              <!-- 规格选择器 -->
              <div v-else class="spec-selector">
                <div class="spec-header">
                  <div class="spec-header-left">
                    <el-icon class="spec-icon" color="#409eff"><Setting /></el-icon>
                    <span class="spec-title">商品规格</span>
                    <el-tag size="small" type="info" class="spec-count">{{ specificationGroups.length }} 个规格类型</el-tag>
                  </div>
                ...
                <div class="spec-categories">
                  <div 
                    v-for="(category, categoryIndex) in groupedSpecificationsByCategory" 
                  ...
                  >
                    <div class="spec-category-header">
                      <div class="spec-category-title">
                        ...
                        <el-tag size="small" type="success">{{ category.totalValues }} 个选项</el-tag>
                      </div>
                      <el-button
                        type="danger"
                        size="small"
        ...
        <el-card class="form-section" shadow="never">
          ...
          <el-form-item label="商品图片">
            <div class="image-upload-container">
              ...
              <!-- 主图设置 -->
              <div v-if="imageList.length > 0" class="main-image-section">
                <el-divider content-position="left">主图设置</el-divider>
                <div class="main-image-grid">
                  ...
                  >
                    ...
                    <div class="image-overlay">
                      <el-icon v-if="image.isMain" class="main-icon"><Star /></el-icon>
        ...
        <el-card class="form-section" shadow="never">
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="6">
              <el-form-item label="商品体积(m³)" prop="volume">
                <el-input-number
        ...
        <el-card class="form-section" shadow="never">
          ...
          <el-row :gutter="24">
            <el-col :span="6">
              <el-form-item label="销售状态" prop="status">
                <el-radio-group v-model="formData.status">
                  ...
                  <el-radio :label="0">暂停销售</el-radio>
                </el-radio-group>
              </el-form-item>
            </el-col>
            <el-col :span="6">
            ...
            <el-col :span="6">
              <el-form-item label="热销商品" prop="isHot">
                <el-switch
  ...
  >
    <template #header>
      <div class="dialog-header">
        <el-icon class="dialog-icon" color="#409eff"><Setting /></el-icon>
        <span>选择商品规格</span>
      </div>
    </template>
    ...
    <div class="specification-dialog">
      ...
      <div class="spec-form-container">
        <el-form :model="specFormData" label-width="100px" class="spec-form">
          <!-- 规格分类选择 -->
            ...
            >
              ...
              >
                <div class="spec-option-content">
                  ...
                  <span class="spec-option-desc" v-if="category.description">
                ...
                >
                  <div class="spec-item-header">
                    <div class="spec-item-info">
                      ...
                      <el-tag v-if="item.unit" size="small" type="info">{{ item.unit }}</el-tag>
                    </div>
                  ...
                  <div v-if="item.values && item.values.length > 0" class="spec-values-selection">
                    <el-checkbox-group 
                  ...
                  <div v-else class="spec-no-values">
                    <el-alert
                      title="该规格项暂无可选值"
                      type="info"
                      :closable="false"
          ...
          <!-- 选择提示 -->
          <div v-if="!specFormData.selectedCategoryId && !loadingSpecs" class="spec-form-hint">
            <el-alert
              title="请先选择规格分类"
          ...
          <!-- 无规格项提示 -->
          <div v-if="specFormData.selectedCategoryId && currentCategoryItems.length === 0 && !loadingSpecItems" class="spec-form-hint">
            <el-alert
              title="该分类暂无规格项"
          ...
          <!-- 已选择的规格总览 -->
          <div v-if="getTotalSelectedValues() > 0" class="selected-values-summary">
            <div class="summary-header">
              <el-icon><Check /></el-icon>
              <span>已选择规格总览</span>
              <el-tag size="small" type="success">{{ getTotalSelectedValues() }} 个规格值</el-tag>
            </div>
      ...
      <!-- 加载状态 -->
      <div v-if="loadingSpecs" class="spec-loading">
        <el-icon class="is-loading"><Loading /></el-icon>
        <span>正在加载规格数据...</span>
      </div>

      <!-- 空状态 -->
      <div v-if="!loadingSpecs && specificationCategories.length === 0" class="spec-dialog-empty">
        <el-empty 
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="specificationDialogVisible = false" size="large">
          取消
        </el-button>
        <el-button 
          type="primary" 
  ...
  goodsCode: [
    ...
    { 
      validator: (rule, value, callback) => {
        // 编辑模式下，如果编码没有改变则跳过验证
        if (isEdit.value && value === originalGoodsCode.value) {
          console.log('规则验证：编辑模式且编码未改变，跳过验证')
          callback()
          return
        }
        // 否则调用原验证函数
...
async function validateGoodsCode(rule, value, callback) {
  ...
  // 编辑模式下，如果编码没有改变则跳过验证
  if (isEdit.value && value === originalGoodsCode.value) {
    console.log('编辑模式：商品编码未改变，跳过验证', { 
      isEdit: isEdit.value, 
      currentValue: value, 
      originalValue: originalGoodsCode.value 
    })
    callback()
    return
  }
...
onMounted(async () => {
  // 检查权限
  if (isEdit.value && !hasPermission(PERMISSIONS.GOODS.GOODS.EDIT)) {
    ElMessage.error('您没有编辑商品的权限')
    router.back()
    return
  }
  
  if (!isEdit.value && !hasPermission(PERMISSIONS.GOODS.GOODS.CREATE)) {
...
const onValueSelectionChange = () => {
  ...
  console.log('规格值选择变化:', specFormData.value.selectedValuesByItem)
}
